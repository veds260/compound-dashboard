...
function convertToLocalTimezone(utcDate: Date | string, timezone?: string | null): Date {
            ...
            <PremiumCard className="animate-fade-in bg-theme-card border-theme-border" gradient>
              ...
              <div className="px-6 pt-4 pb-2">
                <div className="flex items-center space-x-2 overflow-x-auto">
                  {['ALL', 'PENDING', 'SUGGEST_CHANGES', 'APPROVED', 'REJECTED', 'PUBLISHED'].map((filter) => (
                    <button
                      key={filter}
                    ...
                    >
                      {filter === 'ALL' ? 'All' : filter === 'SUGGEST_CHANGES' ? 'Suggest Changes' : filter.charAt(0) + filter.slice(1).toLowerCase()}
                      {filter === 'PENDING' && pendingCount > 0 && ` (${pendingCount})`}
                      {filter === 'SUGGEST_CHANGES' && suggestChangesCount > 0 && ` (${suggestChangesCount})`}
                    </button>
                  ))}
                </div>
              </div>
